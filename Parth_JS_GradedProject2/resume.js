...
let currentIndex = 0;
let filteredResumes = resumes.resume;

function displayApplicant(index) {
    const applicant = filteredResumes[index];
    
    document.getElementById("applicant-name").innerText = applicant.basics.name;
    document.getElementById("applied-for").innerText = `Applied For: ${applicant.basics.AppliedFor}`;
    
    document.getElementById("phone").innerText = applicant.basics.phone;
    document.getElementById("email").innerText = applicant.basics.email;
    document.getElementById("linkedin").href = applicant.basics.profiles.url;
    
    document.getElementById("skills-list").innerHTML = applicant.skills.keywords.map(skill => `<li>${skill}</li>`).join('');
    document.getElementById("hobbies-list").innerHTML = applicant.interests.hobbies.map(hobby => `<li>${hobby}</li>`).join('');
    
    document.getElementById("work-company-name").innerText = `Company Name: ${applicant.work["Company Name"]}`;
    document.getElementById("work-position").innerText = `Position: ${applicant.work.Position}`;
    document.getElementById("work-start-end-dates").innerText = `Start Date: ${applicant.work["Start Date"]}, End Date: ${applicant.work["End Date"]}`;
    document.getElementById("work-summary").innerText = applicant.work.Summary;
    
    document.getElementById("project-description").innerText = applicant.projects.description;
    
    document.getElementById("education-ug").innerText = `UG: ${applicant.education.UG.institute}, ${applicant.education.UG.course}, Start Date: ${applicant.education.UG["Start Date"]}, End Date: ${applicant.education.UG["End Date"]}, CGPA: ${applicant.education.UG.cgpa}`;
    document.getElementById("education-senior-secondary").innerText = `Senior Secondary: ${applicant.education["Senior Secondary"].institute}, CGPA: ${applicant.education["Senior Secondary"].cgpa}`;
    document.getElementById("education-high-school").innerText = `High School: ${applicant.education["High School"].institute}, CGPA: ${applicant.education["High School"].cgpa}`;
    
    document.getElementById("internship-company-name").innerText = `Company Name: ${applicant.Internship["Company Name"]}`;
    document.getElementById("internship-position").innerText = `Position: ${applicant.Internship.Position}`;
    document.getElementById("internship-start-end-dates").innerText = `Start Date: ${applicant.Internship["Start Date"]}, End Date: ${applicant.Internship["End Date"]}`;
    document.getElementById("internship-summary").innerText = applicant.Internship.Summary;
    
    document.getElementById("achievements-summary").innerText = applicant.achievements.Summary.join(", ");
    
    document.getElementById("prev-btn").style.display = index === 0 ? "none" : "inline-block";
    document.getElementById("next-btn").style.display = index === filteredResumes.length - 1 ? "none" : "inline-block";
}
...
function filterByJob() {
    ...
    filteredResumes = resumes.resume.filter(applicant => {
        return applicant.basics.AppliedFor.toLowerCase().includes(searchTerm) ||
            applicant.skills.keywords.some(skill => skill.toLowerCase().includes(searchTerm)) ||
            applicant.work.Summary.toLowerCase().includes(searchTerm) ||
            applicant.projects.description.toLowerCase().includes(searchTerm) ||
            applicant.education.UG.course.toLowerCase().includes(searchTerm) ||
            applicant.education["Senior Secondary"].institute.toLowerCase().includes(searchTerm) ||
            applicant.education["High School"].institute.toLowerCase().includes(searchTerm) ||
            applicant.Internship.Summary.toLowerCase().includes(searchTerm) ||
            applicant.achievements.Summary.join(" ").toLowerCase().includes(searchTerm) ||
            applicant.interests.hobbies.join(" ").toLowerCase().includes(searchTerm);
    });

    if (filteredResumes.length > 0) {
        document.getElementById("no-results").style.display = "none"; // Hide the no-results message
        document.getElementById("resume-container").style.display = "block";
        currentIndex = 0;
        displayApplicant(currentIndex);
    } else {
        document.getElementById("resume-container").style.display = "none"; // Hide the resume content
        document.getElementById("no-results").style.display = "flex"; // Show the no-results message
